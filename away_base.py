...
class AwayBase(plugins.Plugin):
    ...
    try:
        ...
        quote_indices = [m.start() for m in re.finditer(r'"', l)]
        if len(quote_indices) >= 2:
            current_network = l[quote_indices[0] + 1 : quote_indices[1]]
        else:
            current_network = None
    except Exception as e:
        _log(f"[on_epoch] Failed to parse current network: {e}")
        current_network = None

    if current_network and current_network in self.used_networks:
        if self.used_networks[current_network] > self.options['disconnect_after_cycles']:
